...
export default function ProfileScreen() {
  ...
  useEffect(() => {
    if (me) { setName(me.name || ''); setCity(me.city || ''); }
  }, [me]);
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      ...
      >
        ...
        <LinearGradient colors={['#0F172A', '#1E293B']} style={styles.heroGradient}>
          ...
          <View style={styles.trustSection}>
            ...
            <View style={styles.trustBarBg}>
              <Animated.View style={[styles.trustBarFill, { width: `${trustScore}%`, backgroundColor: trustColor }]} />
            </View>
        ...
        {/* ─── Recent Matches (merged from Matches tab) ─── */}
          ...
          {!matches?.length ? (
            ...
            matches.slice(0, 3).map((m: any) => (
              <View key={m.id} style={[styles.matchRow, { borderBottomColor: colors.border }]}>
                ...
                <View style={{ flex: 1 }}>
                  <Text style={[styles.matchName, { color: colors.text }]} numberOfLines={1}>{m.title || `Match #${m.id}`}</Text>
                  <Text style={[styles.matchMeta, { color: colors.mutedForeground }]}>{m.status} · {m.city || 'Unknown'}</Text>
                </View>
                <View style={[styles.matchStatusBadge, {
                  backgroundColor: m.status === 'COMPLETED' ? '#10B98120' : m.status === 'LIVE' ? '#EF444420' : '#3B82F620'
                }]}>
                  <Text style={[styles.matchStatusText, {
                    color: m.status === 'COMPLETED' ? '#10B981' : m.status === 'LIVE' ? '#EF4444' : '#3B82F6'
                  }]}>{m.status}</Text>
                </View>
              </View>
            ))
              ...
              {activeActivity === 'matches' ? (
                !matches?.length ? (
                  ...
                  <View key={m.id} style={[styles.activityItem, { borderColor: colors.border }]}>
                    ...
                    <View style={{ flex: 1 }}>
                      <Text style={[styles.activityTitle, { color: colors.text }]}>{m.title || `Match #${m.id}`}</Text>
                      <Text style={[styles.activitySub, { color: colors.mutedForeground }]}>{m.city || 'Unknown city'}</Text>
                      {m.teamAScore != null && (
                        <View style={styles.scoreRow}>
                          <Text style={[styles.scoreText, { color: colors.primary }]}>{m.teamAScore} – {m.teamBScore}</Text>
                        </View>
                      )}
                    </View>
                    <View style={[styles.matchStatusBadge, {
...
const styles = StyleSheet.create({
  ...
  heroGradient: { paddingHorizontal: 20, paddingBottom: 24, marginBottom: 0 },
